...
export class PerformanceMonitor {
  ...
  logTimer(name: string, context?: string): number {
    ...
    if (this.enabled && duration > 0) {
      console.log(`[Performance] ${name}: ${duration.toFixed(2)}ms${context ? ` (${context})` : ''}`);
    }
    return duration;
  }
  ...
  measureMemory(): void {
    ...
    if (performance.memory) {
      ...
      console.log(`[Memory] Used: ${(usedJSHeapSize / 1024 / 1024).toFixed(2)}MB, Total: ${(totalJSHeapSize / 1024 / 1024).toFixed(2)}MB, Limit: ${(jsHeapSizeLimit / 1024 / 1024).toFixed(2)}MB`);
    }
  }
...
export function measureContentExtraction<T>(name: string, fn: () => T | Promise<T>): T | Promise<T> {
