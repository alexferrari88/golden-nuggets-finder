...
import { mockChrome, mockCrypto } from '../../tests/setup';
import { AccessContext, securityManager } from './security';
...
describe('StorageManager', () => {
  ...
  describe('API Key Management', () => {
    it('should get API key from storage (legacy plain text)', async () => {
      ...
      const context: AccessContext = { source: 'background', action: 'read', timestamp: Date.now() };
    ...
    it('should throw error for invalid access context', async () => {
      const invalidContext: AccessContext = { source: 'content', action: 'write', timestamp: Date.now() };
      await expect(storageManager.getApiKey(invalidContext)).rejects.toThrow('Access denied');
    });
    ...
    it('should save API key to storage (encrypted)', async () => {
      ...
      const context: AccessContext = { source: 'background', action: 'write', timestamp: Date.now() };
