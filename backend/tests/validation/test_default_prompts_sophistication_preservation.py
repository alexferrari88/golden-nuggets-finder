...
class TestDefaultPromptsSophisticationPreservation:
    ...
    def test_chrome_extension_prompt_has_all_sophisticated_features(
        self, optimization_service, chrome_extension_default_prompt_features
    ):
        ...
        for (
            feature_name,
            feature_text,
        ) in chrome_extension_default_prompt_features.items():
            if feature_text not in prompt:
                missing_features.append(f"{feature_name}: '{feature_text}'")

        assert len(missing_features) == 0, (
            f"Chrome extension prompt missing sophisticated features: {missing_features}"
        )

    def test_baseline_prompt_lacks_sophisticated_features(
        self, optimization_service, chrome_extension_default_prompt_features
    ):
        ...
        assert len(found_features) == 0, (
            f"Baseline prompt should not contain sophisticated features: {found_features}"
        )

    @pytest.mark.asyncio
    async def test_optimization_preserves_diamond_miner_principle(
        self, optimization_service, mock_db_with_chrome_feedback, caplog
    ):
...
"""
    ...
    @pytest.mark.asyncio
    async def test_optimization_preserves_anti_patterns(
        self, optimization_service, mock_db_with_chrome_feedback
    ):
...
"""
    ...
    @pytest.mark.asyncio
    async def test_optimization_preserves_quality_control(
        self, optimization_service, mock_db_with_chrome_feedback
    ):
...
"""
    ...
    @pytest.mark.asyncio
    async def test_optimization_preserves_precision_over_recall(
        self, optimization_service, mock_db_with_chrome_feedback
    ):
...
"""
            ...
            assert found_indicators >= 3, (
                f"Should preserve at least 3 precision indicators, found {found_indicators}"
            )

    @pytest.mark.asyncio
    async def test_fallback_preserves_all_sophistication(
        self, optimization_service, mock_db_with_chrome_feedback
    ):
        ...
        with patch.object(optimization_service, "_run_dspy_optimization") as mock_dspy:
            ...
            assert len(missing_features) == 0, (
                f"Fallback missing sophisticated features: {missing_features}"
            )
    ...
    def test_prompt_complexity_metrics_validation(
        self, optimization_service, chrome_extension_default_prompt_features
    ):
        ...
        assert len(prompt) > 3000, (
            f"Prompt should be substantial length, got {len(prompt)}"
        )
        prompt_lines = prompt.split("\n")
        assert len(prompt_lines) > 60, (
            f"Prompt should have many lines, got {len(prompt_lines)}"
        )
        assert prompt.count("##") >= 5, (
            f"Prompt should have multiple sections, got {prompt.count('##')}"
        )
        ...
        assert bad_count == good_count, (
            f"Should have equal bad/good examples: {bad_count} vs {good_count}"
        )
        ...
        assert feature_density > 2.0, (
            f"Feature density too low: {feature_density:.2f} features per 100 words"
        )

    @pytest.mark.asyncio
    async def test_end_to_end_sophistication_preservation(
        self,
        optimization_service,
        mock_db_with_chrome_feedback,
        chrome_extension_default_prompt_features,
    ):
...
"""
            ...
            validation = optimization_service._validate_optimized_prompt_quality(
                original_prompt, optimized_prompt, "end_to_end_test"
            )

            assert validation["quality_preserved"] is True, (
                f"Quality validation failed: {validation['details']}"
            )
            ...
            feature_preservation_rate = preserved_count / len(
                chrome_extension_default_prompt_features
            )
            assert feature_preservation_rate > 0.85, (
                f"Should preserve >85% of features, got {feature_preservation_rate:.2%}"
            )

    @pytest.mark.parametrize("optimization_mode", ["cheap", "expensive"])
    @pytest.mark.asyncio
    async def test_sophistication_preservation_across_modes(
        self, optimization_service, mock_db_with_chrome_feedback, optimization_mode
    ):
...
"""
    ...
    def test_sophisticated_vs_baseline_prompt_differentiation(
        ...
        chrome_score = sum(
            1 for marker in sophisticated_markers if marker in chrome_prompt
        )
        baseline_score = sum(
            1 for marker in sophisticated_markers if marker in baseline_prompt
        )

        assert chrome_score >= 8, (
            f"Chrome prompt should score high on sophistication: {chrome_score}/10"
        )
        assert baseline_score <= 2, (
            f"Baseline prompt should score low on sophistication: {baseline_score}/10"
        )
