...
from app.services.dspy_multi_model_manager import DSPyMultiModelManager, dspy_multi_model_manager
...
class TestDSPyMultiModelManager:
    ...
    def test_manager_initialization(self, manager):
        ...
        for provider_id, prompt in manager.baseline_prompts.items():
            ...
            assert provider_id.replace('_', ' ').title() in prompt or provider_id in prompt
    ...
    @patch('app.services.dspy_multi_model_manager.DSPY_AVAILABLE', False)
    async def test_optimize_for_provider_dspy_unavailable(self, manager, mock_db):
        ...
        with pytest.raises(Exception, match="DSPy not available"):
            await manager.optimize_for_provider(mock_db, 'openai', 'cheap', False)
    ...
    async def test_optimize_for_provider_invalid_provider(self, manager, mock_db):
        ...
        with pytest.raises(ValueError, match="Unsupported provider"):
            await manager.optimize_for_provider(mock_db, 'invalid_provider', 'cheap', False)
    ...
    async def test_get_provider_feedback(self, manager, mock_db):
        ...
        nugget_feedback_data = [
            ('content1', 'tool', 'explanation', 'positive', 'context1', 'url1', 'openai', 'gpt-4o-mini', '2025-01-31T12:00:00Z'),
            ('content2', 'media', None, 'negative', 'context2', 'url2', 'openai', 'gpt-4o-mini', '2025-01-31T13:00:00Z')
        ]
        
        missing_feedback_data = [
            ('missing1', 'analogy', 'context3', 'url3', 'openai', 'gpt-4o-mini', '2025-01-31T14:00:00Z')
        ]
    ...
    async def test_should_optimize_provider(self, manager, mock_db):
        ...
        mock_cursor1 = AsyncMock()
        mock_cursor1.fetchone.return_value = (75, 20, '2025-01-31T12:00:00Z')  # total, negative, last_feedback
        
        mock_cursor2 = AsyncMock()
        mock_cursor2.fetchone.return_value = ('2025-01-24T12:00:00Z',)  # last_optimization (7 days ago)
        ...
        assert result['should_optimize'] == True  # 75 feedback >= 50 threshold
    ...
    async def test_should_optimize_provider_insufficient_feedback(self, manager, mock_db):
        """Test provider optimization with insufficient feedback"""
        # Mock database responses - insufficient feedback
        mock_cursor1 = AsyncMock()
        mock_cursor1.fetchone.return_value = (25, 5, '2025-01-31T12:00:00Z')  # Only 25 feedback items
        ...
        # Verify results
        assert result['should_optimize'] == False
        assert result['total_feedback'] == 25
        assert result['threshold_met'] == False  # 25 < 50 threshold
    ...
    def test_get_fallback_result(self, manager):
        ...
        result = manager._get_fallback_result('openai', feedback_data, 'cheap', 'Test error')
    ...
    def test_extract_provider_prompt_with_demos(self, manager):
        ...
        # Mock demonstrations
        demo1 = Mock()
        demo1.content = "Test input content for demo 1" * 10  # Make it long to test truncation
        demo1.golden_nuggets = '{"golden_nuggets": [{"type": "tool", "content": "demo output"}]}'
    ...
    def test_log_progress(self, manager):
        ...
        manager._log_progress(provider_id, run_id, 'optimization', 50, 'Test progress message')
    ...
    def test_get_all_provider_active_runs(self, manager):
        ...
        manager.active_runs_by_provider = {
            'openai': {
                'run1': {'step': 'optimization', 'progress': 30, 'last_updated': datetime.now(timezone.utc)},
                'run2': {'step': 'evaluation', 'progress': 80, 'last_updated': datetime.now(timezone.utc)}
            },
            'gemini': {
                'run3': {'step': 'storing', 'progress': 95, 'last_updated': datetime.now(timezone.utc)}
            }
        }
    ...
    async def test_store_provider_optimized_prompt(self, manager, mock_db):
        ...
        assert mock_db.execute.call_count == 3  # version query, update current, insert new
...
class TestAsyncFunctionality:
    ...
    async def test_async_get_provider_current_prompt_found(self):
        ...
        mock_data = ('prompt-id-123', 5, 'Optimized prompt text', '2025-01-31T12:00:00Z', 100, 0.85, 'claude-3-5-sonnet')
        mock_cursor = AsyncMock()
