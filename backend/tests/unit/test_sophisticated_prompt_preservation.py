...
class TestSophisticatedPromptPreservation:
    ...
    def test_chrome_extension_prompt_contains_all_features(
        self, optimization_service, sophisticated_prompt_features
    ):
        ...
        for feature_name, feature_text in sophisticated_prompt_features.items():
            assert feature_text in prompt, (
                f"Missing sophisticated feature: {feature_name} ('{feature_text}')"
            )

    def test_baseline_prompt_lacks_sophistication(
        self, optimization_service, sophisticated_prompt_features
    ):
        ...
        for feature in sophisticated_features:
            assert feature not in prompt, (
                f"Baseline should not have sophisticated feature: {feature}"
            )
    ...
    def test_quality_validation_complete_loss(self, optimization_service):
        ...
        original = optimization_service.chrome_extension_default_prompt
        simplified = (
            "You are helpful. Extract insights. Return JSON with golden_nuggets array."
        )
    ...
    def test_quality_validation_case_insensitive_preservation(
...
"""
    ...
    def test_diamond_miner_principle_components(self, optimization_service):
        ...
        for component in key_components:
            assert component in principle_text, (
                f"Diamond Miner Principle missing component: {component}"
            )
    ...
    def test_precision_over_recall_philosophy(self, optimization_service):
        ...
        for indicator in precision_indicators:
            assert indicator in prompt, (
                f"Missing precision over recall indicator: {indicator}"
            )
    ...
    def test_prompt_analysis_parametrized(
        self, optimization_service, feature, expected
    ):
...
Return valid JSON with the exact structure: {{"golden_nuggets": [...]}}"""
        ...
        assert (
            f"Based on {len(training_examples)} user feedback examples"
            in enhanced_prompt
        )
    ...
    def test_prompt_length_and_complexity_metrics(self, optimization_service):
        ...
        assert (
            len(chrome_prompt) > len(baseline_prompt) * 3
    ...
    def test_example_structure_consistency(self, optimization_service):
        ...
        for i, bad_pos in enumerate(bad_positions):
            assert i < len(good_positions), (
                f"Bad example {i} not paired with good example"
            )
            assert good_positions[i] > bad_pos, (
                f"Good example {i} should come after bad example {i}"
            )
