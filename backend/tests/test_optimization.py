...
from app.services.dspy_config import (
    DSPyConfig, validate_dspy_environment, generate_mock_feedback_data,
    OptimizationMetrics
)
...
class TestDSPyConfiguration:
    ...
    def test_optimization_metrics(self):
        ...
        # Test perfect match
        example = Mock()
        example.golden_nuggets = json.dumps({"golden_nuggets": [{"type": "tool", "content": "test"}]})
        
        pred = Mock()
        pred.golden_nuggets = json.dumps({"golden_nuggets": [{"type": "tool", "content": "test"}]})
...
class TestOptimizationService:
    ...
    async def test_training_example_generation(self, setup_database):
        ...
        async with get_db() as db:
            ...
            training_examples = await feedback_service.get_training_examples(db, limit=50)
    ...
    async def test_optimization_modes(self, setup_database):
        ...
        async with get_db() as db:
            ...
            for mode in modes:
                try:
                    result = await service.run_optimization(db, mode, auto_trigger=False)
...
class TestOptimizationIntegration:
    ...
    async def test_concurrent_optimization_handling(self, setup_database):
        ...
        async with get_db() as db:
            ...
            tasks = []
            for i in range(3):  # More tasks than workers
