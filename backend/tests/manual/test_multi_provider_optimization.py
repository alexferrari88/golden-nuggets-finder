...
async def create_sample_feedback(db, provider_id: str, model_name: str, count: int = 60):
    ...
    for i in range(count):
        ...
        await db.execute(
            ...
            (
                ...
                feedback_item['type'],
                'explanation' if feedback_item['rating'] == 'negative' and i % 10 == 0 else None,
...
async def test_provider_optimization_thresholds():
    ...
    async with get_db() as db:
        ...
        threshold_results = await optimization_service.check_provider_optimization_thresholds(db)
        ...
        for provider_id, data in threshold_results.items():
            ...
            print(f"             | Threshold met: {data['threshold_met']} | Negative rate: {data['negative_rate']:.2%}")
            print(f"             | Days since optimization: {data['days_since_optimization']}")
            print("-" * 60)
...
async def test_provider_specific_optimization(provider_id: str):
    ...
    async with get_db() as db:
        ...
        try:
            ...
            print(f"   Performance improvement: {result.get('performance_improvement', 0):.3f}")
            print(f"   Training examples: {result.get('training_examples')}")
            
            # Get the optimized prompt
            optimized_prompt = await optimization_service.get_provider_current_prompt(db, provider_id)
            if optimized_prompt:
...
async def test_multi_provider_manager_directly():
    ...
    async with get_db() as db:
        ...
        for provider_id in ['gemini', 'openai']:
            feedback = await dspy_multi_model_manager._get_provider_feedback(db, provider_id)
        ...
        for provider_id in ['gemini', 'openai']:
            should_optimize = await dspy_multi_model_manager.should_optimize_provider(db, provider_id)
            print(f"   {provider_id} should optimize: {should_optimize['should_optimize']}")
...
async def test_progress_tracking():
    ...
    if progress:
        print(f"   ✅ Progress tracking working: {progress['step']} at {progress['progress']}%")
...
async def test_fallback_behavior():
    ...
    extracted_prompt = dspy_multi_model_manager._extract_provider_prompt(mock_module, 'gemini')
    print(f"   ✅ Prompt extraction fallback working")
...
async def main():
    ...
    if not os.environ.get('FORCE_TEST_DB'):
        print("❌ ERROR: Must run with FORCE_TEST_DB=1 to prevent production database pollution")
        sys.exit(1)
    ...
    try:
        ...
        async with get_db() as db:
            ...
            await create_sample_feedback(db, 'anthropic', 'claude-3-5-sonnet', 30)  # Below threshold
        ...
        providers_to_test = [
            provider_id for provider_id, data in threshold_results.items() 
        ...
        if providers_to_test:
            ...
            for provider_id in providers_to_test[:2]:  # Limit to 2 providers for testing
        ...
        # Summary
        print("\n📋 Test Summary:")
        print(f"   - Sample feedback created for 4 providers")
        print(f"   - Threshold checking: ✅")
        print(f"   - Multi-model manager: ✅")
