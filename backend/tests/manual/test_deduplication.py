...
async def test_deduplication():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.post(
            f"{BASE_URL}/feedback", json=missing_content_data
        ) as response:
        ...
        missing_content_data["missingContentFeedback"][0]["id"] = str(
            uuid.uuid4()
        )  # New ID
        ...
        async with session.get(f"{BASE_URL}/feedback/duplicates") as response:
            if response.status == 200:
                ...
                for i, dup in enumerate(
                    duplicates.get("duplicates", [])[:3]
                ):  # Show first 3
                    print(
                        f"   {i + 1}. {dup['feedback_type']} - Count: {dup['report_count']}"
                    )
        ...
        async with session.get(f"{BASE_URL}/dashboard/stats") as response:
            if response.status == 200:
                ...
                print(
                    f"   Total missing reports: {stats.get('total_missing_reports', 'N/A')}"
                )
                print(
                    f"   Duplicate nugget items: {stats.get('duplicate_nugget_items', 'N/A')}"
                )
                print(
                    f"   Duplicate missing items: {stats.get('duplicate_missing_items', 'N/A')}"
                )
            else:
...
async def test_edge_cases():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.post(f"{BASE_URL}/feedback", json=base_data) as response:
            result1 = await response.json()
            print(
                f"   URL1: {result1.get('deduplication', {}).get('missing_content_duplicates', 0)} duplicates"
            )
        ...
        async with session.post(f"{BASE_URL}/feedback", json=base_data) as response:
            result2 = await response.json()
            print(
                f"   URL2: {result2.get('deduplication', {}).get('missing_content_duplicates', 0)} duplicates"
            )
        ...
        async with session.post(f"{BASE_URL}/feedback", json=nugget_base) as response:
            result1 = await response.json()
            print(
                f"   Type 'tool': {result1.get('deduplication', {}).get('nugget_duplicates', 0)} duplicates"
            )
