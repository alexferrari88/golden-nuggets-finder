...
5.  **Mental Models:** A named cognitive framework, productivity technique, or principle for thinking. The simple mention of a specific model is valuable as a hook for further research.
    ...
    *   **Good:** "I apply the 'Inversion' mental model by asking 'What would guarantee failure?' before starting a new project. This helps me identify and mitigate risks proactively instead of just planning for success." """
    ...
    def test_chrome_extension_prompt_characteristics(self, optimization_service, chrome_extension_default_prompt):
    ...
    @pytest.mark.asyncio
    async def test_optimization_uses_chrome_extension_prompt(self, optimization_service, mock_db_with_feedback, caplog):
        ...
        with patch.object(optimization_service, '_run_dspy_optimization') as mock_dspy:
            ...
            mock_dspy.return_value = {
                "optimized_prompt": "OPTIMIZED: " + optimization_service.chrome_extension_default_prompt,
            ...
            assert "Chrome extension prompt" in log_messages or "sophisticated" in log_messages
            assert "optimizing_chrome_prompt" in log_messages or "chrome_extension_sophisticated" in log_messages

    @pytest.mark.asyncio
    async def test_prompt_quality_validation(self, optimization_service, mock_db_with_feedback):
    ...
    @pytest.mark.asyncio
    async def test_fallback_preserves_sophistication(self, optimization_service, mock_db_with_feedback):
        ...
        with patch.object(optimization_service, '_run_dspy_optimization') as mock_dspy:
            ...
            result = optimization_service._run_dspy_optimization([], "cheap", "test_run")
    ...
    @pytest.mark.asyncio
    async def test_provider_specific_optimization_chrome_prompts(self, optimization_service, mock_db_with_feedback):
        ...
        for provider_id in providers:
            with patch.object(optimization_service.multi_model_manager, 'optimize_for_provider') as mock_optimize:
                ...
                mock_optimize.assert_called_once_with(mock_db_with_feedback, provider_id, "cheap", False)
    ...
    @pytest.mark.asyncio
    async def test_multiple_chrome_prompts_optimization(self, optimization_service, mock_db_with_feedback):
        ...
        for i in range(3):
            with patch.object(optimization_service, '_run_dspy_optimization') as mock_dspy:
                mock_dspy.return_value = {
                    "optimized_prompt": f"OPTIMIZED_{i}: " + optimization_service.chrome_extension_default_prompt,
    ...
    @pytest.mark.asyncio
    async def test_backward_compatibility_with_existing_feedback(self, optimization_service):
        ...
        with patch.object(optimization_service, '_run_dspy_optimization') as mock_dspy:
            mock_dspy.return_value = {
                "optimized_prompt": optimization_service.chrome_extension_default_prompt + " # Optimized",
                "performance_score": 0.85,
                "baseline_score": 0.70,
                "improvement": 0.15,
                "execution_time": 120.0,
    ...
    def test_chrome_extension_vs_baseline_prompt_differences(self, optimization_service):
    ...
    @pytest.mark.asyncio 
    async def test_optimization_preserves_diamond_miner_principle(self, optimization_service, mock_db_with_feedback):
        ...
        with patch.object(optimization_service, '_run_dspy_optimization') as mock_dspy:
            ...
            optimized_with_principle = optimization_service.chrome_extension_default_prompt + """
