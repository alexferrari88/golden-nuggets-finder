...
class TestProviderSpecificOptimization:
    ...
    @pytest.mark.asyncio
    async def test_provider_optimization_triggers(self, optimization_service, mock_db_with_provider_feedback, providers):
        """Test that provider-specific optimization can be triggered for each provider"""
        
        for provider_id in providers:
            with patch.object(optimization_service.multi_model_manager, 'optimize_for_provider') as mock_optimize:
    ...
    @pytest.mark.asyncio
    async def test_provider_threshold_checking(self, optimization_service, mock_db_with_provider_feedback, providers):
        """Test threshold checking for each provider"""
        
        with patch.object(optimization_service.multi_model_manager, 'should_optimize_provider') as mock_should_optimize:
            # Mock different threshold states for different providers
            mock_should_optimize.side_effect = lambda db, provider_id: {
                "should_optimize": provider_id in ["gemini", "anthropic"],  # Only some meet threshold
    ...
    @pytest.mark.asyncio
    async def test_auto_trigger_provider_optimizations(self, optimization_service, mock_db_with_provider_feedback):
        """Test automatic triggering of provider optimizations"""
        
        with patch.object(optimization_service, 'check_provider_optimization_thresholds') as mock_check:
            with patch.object(optimization_service, 'run_provider_optimization') as mock_optimize:
                
                # Mock threshold results - some providers meet threshold
                mock_check.return_value = {
                    "gemini": {"should_optimize": True, "total_feedback": 30, "threshold_met": True},
                    "openai": {"should_optimize": False, "total_feedback": 10, "threshold_met": False},
                    "anthropic": {"should_optimize": True, "total_feedback": 25, "threshold_met": True},
                    "openrouter": {"should_optimize": False, "total_feedback": 5, "threshold_met": False}
                }
                ...
                results = await optimization_service.auto_trigger_provider_optimizations(
                    mock_db_with_provider_feedback
                )
                ...
                triggered_providers = [item["provider_id"] for item in results["triggered"]]
    ...
    @pytest.mark.asyncio
    async def test_provider_current_prompt_retrieval(self, optimization_service, providers):
        """Test retrieving current optimized prompt for each provider"""
        
        mock_db = AsyncMock()
        
        for provider_id in providers:
            with patch.object(optimization_service.multi_model_manager, 'get_provider_current_prompt') as mock_get_prompt:
    ...
    @pytest.mark.asyncio
    async def test_provider_optimization_with_chrome_extension_features(self, optimization_service, mock_db_with_provider_feedback, caplog):
    ...
    @pytest.mark.asyncio
    async def test_provider_optimization_error_handling(self, optimization_service, mock_db_with_provider_feedback):
    ...
    async def test_provider_progress_tracking(self, optimization_service, providers):
        ...
        for provider_id in providers:
            ...
            with patch.object(optimization_service.multi_model_manager, 'get_provider_run_progress') as mock_get_progress:
                ...
                result = optimization_service.get_provider_run_progress(provider_id, run_id)
    ...
    async def test_all_provider_active_runs(self, optimization_service):
        ...
        with patch.object(optimization_service.multi_model_manager, 'get_all_provider_active_runs') as mock_get_all_runs:
            
            mock_active_runs = {
                "gemini": {
                    "run_123": {"step": "optimization", "progress": 60, "message": "Optimizing Gemini prompt"},
                    "run_124": {"step": "validation", "progress": 90, "message": "Validating Gemini optimization"}
                },
                "openai": {
                    "run_125": {"step": "data_gathering", "progress": 30, "message": "Gathering OpenAI training data"}
                },
                "anthropic": {},  # No active runs
                "openrouter": {
                    "run_126": {"step": "storing", "progress": 95, "message": "Storing OpenRouter optimization"}
                }
            }
    ...
    @pytest.mark.parametrize("provider_id,model_name", [
    ...
    @pytest.mark.asyncio
    async def test_provider_model_specific_prompts(self, optimization_service, provider_id, model_name):
        """Test provider+model specific prompt retrieval"""
        
        mock_db = AsyncMock()
        
        with patch.object(optimization_service, 'get_current_prompt_for_provider_model') as mock_get_prompt:
    ...
    @pytest.mark.asyncio
    async def test_provider_optimization_modes(self, optimization_service, mock_db_with_provider_feedback):
    ...
    @pytest.mark.asyncio
    async def test_cross_provider_optimization_independence(self, optimization_service, mock_db_with_provider_feedback):
