...
async def test_dashboard_functionality():
    ...
    async with get_db() as db:
        ...
        try:
            cursor = await db.execute("SELECT * FROM recent_feedback_with_status LIMIT 5")
        ...
        try:
            ...
            print(f"   ✅ Pending feedback: {result['total_count']} total, {len(result['items'])} returned")
        except Exception as e:
        ...
        try:
            ...
            for item in activity[:2]:
                print(f"      - {item['run_id']}: {item['phase']} ({item['progress']}%)")
        except Exception as e:
        ...
        try:
            ...
            print(f"   ✅ Cost summary: ${summary['total_cost']:.3f} over {summary['period_days']} days")
        ...
        try:
            ...
            print(f"   ✅ Usage stats: {usage_stats['total_unique_used']} unique items used")
            print(f"      - Total uses: {usage_stats['total_usage_records']}")
            print(f"      - Avg contribution: {usage_stats['average_contribution']:.2f}")
        except Exception as e:
...
async def create_sample_data():
    ...
    async with get_db() as db:
        ...
        """, (
            run_id, "cheap", "manual", datetime.now().isoformat(), 
            "running", 0, 0, 0.0
        ))
        await db.commit()
        ...
        await progress_service.save_progress(
            db, run_id, "initialization", 10, "🚀 Starting optimization", 
            {"test": True}
        )
        await progress_service.save_progress(
            db, run_id, "data_gathering", 30, "📊 Gathering training data", 
            {"examples_found": 25}
        )
        await progress_service.save_progress(
            db, run_id, "optimization", 70, "🧠 Running DSPy optimization", 
            {"current_score": 0.75}
        )
        ...
        await cost_service.track_operation_cost(
            db, run_id, "optimization", "gemini-2.5-flash", 
            1500, 300, {"operation": "test"}
        )
        await cost_service.track_operation_cost(
            db, run_id, "evaluation", "gpt-4o", 
            2000, 500, {"operation": "test"}
        )
...
async def cleanup_sample_data():
    ...
    async with get_db() as db:
        ...
        await db.execute("DELETE FROM optimization_progress WHERE optimization_run_id = ?", ("test-run-123",))
        await db.execute("DELETE FROM cost_tracking WHERE optimization_run_id = ?", ("test-run-123",))
        await db.execute("DELETE FROM optimization_runs WHERE id = ?", ("test-run-123",))
        await db.commit()
