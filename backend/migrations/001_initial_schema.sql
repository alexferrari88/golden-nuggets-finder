...
CREATE TABLE IF NOT EXISTS nugget_feedback (
    id TEXT PRIMARY KEY,
    nugget_content TEXT NOT NULL,           -- Full golden nugget content
    ...
    context TEXT NOT NULL,                  -- Full surrounding context from page
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP  -- Server timestamp
);

-- Create indexes for nugget_feedback table
CREATE INDEX IF NOT EXISTS idx_nugget_feedback_timestamp ON nugget_feedback(timestamp);
CREATE INDEX IF NOT EXISTS idx_nugget_feedback_rating ON nugget_feedback(rating);
CREATE INDEX IF NOT EXISTS idx_nugget_feedback_url ON nugget_feedback(url);
CREATE INDEX IF NOT EXISTS idx_nugget_feedback_created ON nugget_feedback(created_at);
...
CREATE TABLE IF NOT EXISTS missing_content_feedback (
    ...
    context TEXT NOT NULL,                  -- Full page context
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP  -- Server timestamp
);

-- Create indexes for missing_content_feedback table
CREATE INDEX IF NOT EXISTS idx_missing_content_timestamp ON missing_content_feedback(timestamp);
CREATE INDEX IF NOT EXISTS idx_missing_content_type ON missing_content_feedback(suggested_type);
CREATE INDEX IF NOT EXISTS idx_missing_content_url ON missing_content_feedback(url);
CREATE INDEX IF NOT EXISTS idx_missing_content_created ON missing_content_feedback(created_at);
...
CREATE TABLE IF NOT EXISTS optimization_runs (
    ...
    result_prompt TEXT,                     -- Full optimized prompt text
...
-- Create indexes for optimization_runs table
CREATE INDEX IF NOT EXISTS idx_optimization_runs_started ON optimization_runs(started_at);
CREATE INDEX IF NOT EXISTS idx_optimization_runs_status ON optimization_runs(status);
CREATE INDEX IF NOT EXISTS idx_optimization_runs_mode ON optimization_runs(mode);
...
-- Create indexes for optimized_prompts table
CREATE INDEX IF NOT EXISTS idx_optimized_prompts_version ON optimized_prompts(version);
CREATE INDEX IF NOT EXISTS idx_optimized_prompts_current ON optimized_prompts(is_current);
CREATE INDEX IF NOT EXISTS idx_optimized_prompts_date ON optimized_prompts(optimization_date);
...
-- Create indexes for training_examples table
CREATE INDEX IF NOT EXISTS idx_training_examples_score ON training_examples(feedback_score);
CREATE INDEX IF NOT EXISTS idx_training_examples_timestamp ON training_examples(timestamp);
CREATE INDEX IF NOT EXISTS idx_training_examples_used ON training_examples(used_in_optimization);
