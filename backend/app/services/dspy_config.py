...
Handles DSPy setup, model configuration, and optimization parameters using Gemini 2.5-flash only.
"""
...
class DSPyConfig:
    ...
    def get_language_model(self, model_name: str = "gemini-2.5-flash"):
        ...
        if not DSPY_AVAILABLE:
            raise ImportError("DSPy is not available. Install with: pip install dspy-ai")
        ...
        try:
            ...
            raise Exception(f"Failed to configure DSPy with Gemini {model_name}: {e}")
...
if DSPY_AVAILABLE:
    class GoldenNuggetSignature(dspy.Signature):
        ...
        golden_nuggets = dspy.OutputField(
            desc="JSON object containing extracted golden nuggets with type, content, and synthesis"
        )
    ...
    # Provide placeholder classes when DSPy is not available
    class GoldenNuggetSignature:
        """Placeholder when DSPy is not available"""
        pass
    
    class GoldenNuggetExtractor:
        """Placeholder when DSPy is not available"""
        def __init__(self, signature=None):
            pass
...
class OptimizationMetrics:
    ...
    @staticmethod
    def golden_nugget_metric(example, pred, trace=None) -> float:
        """
        ...
        try:
            ...
            count_score = min(len(pred_nuggets), len(expected_nuggets)) / max(len(pred_nuggets), len(expected_nuggets))
            
            # Calculate type alignment (bonus for correct types)
            type_matches = 0
            expected_types = set(nugget.get('type', '') for nugget in expected_nuggets)
            predicted_types = set(nugget.get('type', '') for nugget in pred_nuggets)
            ...
            expected_content = ' '.join(nugget.get('content', '') for nugget in expected_nuggets).lower()
            predicted_content = ' '.join(nugget.get('content', '') for nugget in pred_nuggets).lower()
            
            expected_words = set(expected_content.split())
            predicted_words = set(predicted_content.split())
            
            if len(expected_words) > 0:
                content_overlap = len(expected_words.intersection(predicted_words)) / len(expected_words)
            else:
                content_overlap = 0.0
...
def validate_dspy_environment() -> Dict[str, Any]:
    ...
    if not DSPY_AVAILABLE:
        status["errors"].append("DSPy is not installed. Install with: pip install dspy-ai")
        return status
    ...
    try:
        ...
        try:
            ...
            status["errors"].append(f"Gemini model test call failed: {str(e)}")
        
    except Exception as e:
        status["errors"].append(f"DSPy configuration failed: {str(e)}")
