...
Handles DSPy optimization separately for each LLM provider (Gemini, OpenAI, Anthropic, OpenRouter)
to ensure model-specific prompt optimization based on feedback data.
"""
...
if not DSPY_AVAILABLE:
    print("Warning: DSPy not available. Multi-model optimization features will be limited.")
...
class DSPyMultiModelManager:
    ...
    def __init__(self):
        ...
        self.executor = ThreadPoolExecutor(max_workers=2)  # Limit concurrent optimizations
...
"""
    ...
    def _initialize_model_configs(self):
        ...
        try:
            ...
            self.model_configs = {
                ...
                'openrouter': {
                    'lm': dspy.LM('deepseek/deepseek-r1', api_base='https://openrouter.ai/api/v1'),
                    'default_model': 'deepseek/deepseek-r1'
                }
            }
            
            logger.info("DSPy model configurations initialized for multi-provider support")
    ...
    async def optimize_for_provider(
        ...
        await db.execute(
            ...
            """,
            (
                run_id, mode, trigger_type, datetime.now(timezone.utc), "running", 
                len(provider_feedback), provider_id, 
        ...
        self._log_progress(provider_id, run_id, "initialization", 10, f"Setting up {provider_id} optimization")
        ...
        try:
            ...
            if provider_id in self.active_runs_by_provider and run_id in self.active_runs_by_provider[provider_id]:
    ...
    def _run_provider_optimization(
        ...
        try:
            ...
            self._log_progress(provider_id, run_id, "data_gathering", 20, "Creating training examples")
            ...
            self._log_progress(provider_id, run_id, "optimization", 40, f"Running {mode} optimization")
            
            if mode == 'expensive':
                from dspy.teleprompt import MIPROv2
                optimizer = MIPROv2(metric=OptimizationMetrics.golden_nugget_metric, num_candidates=10)
            else:
            ...
            self._log_progress(provider_id, run_id, "evaluation", 80, "Evaluating performance")
            ...
            optimized_prompt = self._extract_provider_prompt(optimized_extractor, provider_id)
...
"""
            ...
            if hasattr(module, "extract") and hasattr(module.extract, "signature"):
                ...
                if hasattr(module.extract, "demos") and module.extract.demos:
                    ...
                    for demo in module.extract.demos[:2]:  # Limit to first 2 examples
                        ...
                        prompt_parts.append(f"Output: {str(demo.golden_nuggets)[:150]}...")

                # Add the actual task description
                prompt_parts.append(f"\n# Task for {provider_id}:")
                prompt_parts.append("Extract golden nuggets from the provided content.")
                prompt_parts.append(f"This prompt has been optimized specifically for {provider_id}.")
    ...
    async def _get_provider_feedback(self, db: aiosqlite.Connection, provider_id: str) -> List[Dict]:
    ...
    async def _store_provider_optimized_prompt(
        ...
        await db.execute(
            ...
            (
                ...
                run_id,
                provider_id,
                optimization_result.get("model_name", self.model_configs.get(provider_id, {}).get('default_model', 'unknown'))
            )
        )
    ...
    def _log_progress(self, provider_id: str, run_id: str, step: str, progress: int, message: str):
        ...
        try:
            ...
            if progress == 100:
                logger.info(
                    ...
                    extra={"provider_id": provider_id, "run_id": run_id, "step": step, "progress": progress}
                )
            elif progress == -1:
                logger.error(
                    f"❌ {provider_id}: {message}",
                    extra={"provider_id": provider_id, "run_id": run_id, "step": step, "error": True}
                )
            else:
                logger.info(
    ...
    async def should_optimize_provider(
        ...
        if last_optimization:
            try:
                last_opt_date = datetime.fromisoformat(last_optimization.replace('Z', '+00:00'))
                days_since_optimization = (datetime.now(timezone.utc) - last_opt_date).days
            except:
                days_since_optimization = 999

        # Determine if should optimize
        should_optimize = (
            total_feedback >= self.min_feedback_threshold and (
                days_since_optimization >= 7 or  # Weekly optimization
                negative_rate >= 0.4 or  # High negative rate
                total_feedback >= 150  # High volume
            )
        )
