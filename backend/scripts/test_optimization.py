...
from app.services.dspy_config import (
    validate_dspy_environment, generate_mock_feedback_data, DSPyConfig
)
...
async def test_dspy_environment():
    ...
    if status['configuration_valid']:
        ...
        print("💡 Make sure to set GEMINI_API_KEY with the same key as your Chrome extension")
        return False
...
async def test_optimization(mode: str = "cheap"):
    ...
    async with get_db() as db:
        ...
        if not stats['shouldOptimize']:
            print("⚠️  Optimization thresholds not met, but running anyway for testing...")
        ...
        try:
            ...
            if result.get("success"):
                ...
                print(f"Performance improvement: {result.get('performance_improvement', 0):.1%}")
                ...
                if current_prompt:
                    ...
                    print(f"Performance score: {current_prompt['performance']['positiveRate']:.3f}")
            else:
...
async def benchmark_optimization():
    ...
    for mode in modes:
        for data_size in data_sizes:
            ...
            try:
                ...
                print(f"  Time: {execution_time:.1f}s, Score: {result.get('performance_score', 0):.3f}")
    ...
    for result in results:
        ...
        print(f"{status} {result['mode']:>9} | {result['data_size']:>3} examples | "
...
async def monitor_optimization_history():
    ...
    async with get_db() as db:
        ...
        print(f"{'Date':<19} | {'Mode':<9} | {'Status':<9} | {'Improvement':<11} | {'Examples'}")
        print("-" * 70)
        ...
        for run in history:
            ...
            print(f"{date} | {mode:>9} | {status_icon} {status:<7} | {improvement:>9.1%} | {examples:>8}")
...
async def cleanup_test_data():
    ...
    async with get_db() as db:
        ...
        await db.execute("DELETE FROM training_examples WHERE url LIKE 'https://example.com/test/%'")
        
        # Delete test feedback
        await db.execute("DELETE FROM nugget_feedback WHERE url LIKE 'https://example.com/test/%'")
        await db.execute("DELETE FROM missing_content_feedback WHERE url LIKE 'https://example.com/test/%'")
