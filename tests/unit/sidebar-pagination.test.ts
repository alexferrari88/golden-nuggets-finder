...
vi.mock('../../src/shared/content-reconstruction', () => ({
  getDisplayContent: vi.fn((nugget) => `${nugget.startContent}...${nugget.endContent}`),
  reconstructFullContent: vi.fn((nugget) => `${nugget.startContent}...${nugget.endContent}`)
}));
...
describe('Sidebar Pagination', () => {
  ...
  it('should navigate to second page and show remaining items', () => {
    ...
    const pagination = document.querySelector('.nugget-pagination');
    const nextButton = Array.from(pagination?.querySelectorAll('.pagination-button') || [])
    ...
    const paginationButtons = updatedPagination?.querySelectorAll('.pagination-button');
  ...
  it('should maintain pagination state when refreshing page', () => {
    ...
    // Navigate to page 2
    const nextButton = Array.from(document.querySelectorAll('button')).find((el): el is HTMLButtonElement => 
      el instanceof HTMLButtonElement && el.textContent?.includes('Next')
    );
    nextButton!.click();
    ...
    const container = document.querySelector('#nugget-list-container') as HTMLElement;
