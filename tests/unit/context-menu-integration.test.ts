...
describe('Context Menu Integration Tests', () => {
  ...
  let contextMenuClickHandler: (info: chrome.contextMenus.OnClickData, tab?: chrome.tabs.Tab) => void;
  ...
  async function handleContextMenuClick(info: chrome.contextMenus.OnClickData, tab?: chrome.tabs.Tab): Promise<void> {
    ...
    function parseMenuId(menuId: string) {
      if (menuId === 'report-missed-nugget') {
        return { type: 'missed-nugget', promptId: undefined, typeFilter: undefined };
      }
      ...
      if (!action || action.trim() === '' || !promptId || promptId.trim() === '') return null;
  ...
  describe('Context Menu Creation Integration', () => {
    it('should create hierarchical context menus with type filtering options', async () => {
      ...
      expect(mockChrome.contextMenus.create).toHaveBeenCalledTimes(expectedMenuCount);
  ...
  describe('Context Menu Click Handling Integration', () => {
    ...
    it('should handle missed nugget reporting with tab state validation', async () => {
      ...
      function validateMissedNuggetReport(
        ...
        if (!completedTabs.has(tab.id)) return { valid: false, reason: 'Analysis not completed' };
        
        const selectedText = info?.selectionText?.trim() || '';
        if (selectedText.length <= 5) return { valid: false, reason: 'Selected text too short' };
        
        return { valid: true };
      }

      const validation = validateMissedNuggetReport(tab, clickInfo, analysisCompletedTabs);
      ...
      function parseMenuId(menuId: string) {
        if (!menuId || menuId === 'report-missed-nugget') {
          return menuId === 'report-missed-nugget' 
            ? { type: 'missed-nugget', promptId: undefined, typeFilter: undefined }
            : null;
        }
  ...
  describe('Content Script Injection Integration', () => {
    it('should handle content script injection with proper error handling', async () => {
      ...
      async function injectAndVerifyContentScript(tabId: number): Promise<boolean> {
        try {
          // Check if content script is already injected
          const testResponse = await mockChrome.tabs.sendMessage(tabId, { type: 'PING' }).catch(() => null);
          ...
          const verifyResponse = await mockChrome.tabs.sendMessage(tabId, { type: 'PING' }).catch(() => null);
    ...
    it('should handle injection failures gracefully', async () => {
      ...
      mockChrome.tabs.sendMessage.mockRejectedValueOnce(new Error('No content script')); // First PING fails
      mockChrome.scripting.executeScript.mockRejectedValueOnce(new Error('Permission denied')); // Injection fails
    ...
    it('should detect already injected content scripts', async () => {
      ...
      mockChrome.tabs.sendMessage.mockResolvedValueOnce({ success: true, alreadyInjected: true });
