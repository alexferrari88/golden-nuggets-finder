...
describe('ErrorHandler', () => {
  ...
  describe('Error Classification', () => {
    test('correctly identifies API key errors', async () => {
      ...
      const result = await ErrorHandler.handleProviderError(apiKeyError, 'openai', 'test');
    ...
    test('correctly identifies rate limit errors', async () => {
      ...
      const result = await ErrorHandler.handleProviderError(rateLimitError, 'openai', 'test');
    ...
    test('correctly identifies temporary errors', async () => {
      ...
      const result = await ErrorHandler.handleProviderError(tempError, 'openai', 'test');
    ...
    test('handles serious errors with fallback provider', async () => {
      ...
      vi.mocked(ProviderSwitcher.getCurrentProvider).mockResolvedValue('openai');
      vi.mocked(ProviderSwitcher.getAvailableProviders).mockResolvedValue(['openai', 'gemini']);
      
      const result = await ErrorHandler.handleProviderError(seriousError, 'openai', 'test');
  ...
  describe('Retry Logic', () => {
    test('identifies retry-able errors correctly', async () => {
      ...
      const result = await ErrorHandler.handleProviderError(rateLimitError, 'openai', 'test-simple');
  ...
  describe('Fallback Provider Selection', () => {
    test('selects fallback provider based on priority order', async () => {
      ...
      vi.mocked(ProviderSwitcher.getCurrentProvider).mockResolvedValue('openrouter');
      vi.mocked(ProviderSwitcher.getAvailableProviders).mockResolvedValue(['openrouter', 'anthropic', 'gemini']);
      
      const result = await ErrorHandler.handleProviderError(error, 'openrouter', 'test');
    ...
    test('handles no available fallback providers', async () => {
      ...
      vi.mocked(ProviderSwitcher.getCurrentProvider).mockResolvedValue('gemini');
      vi.mocked(ProviderSwitcher.getAvailableProviders).mockResolvedValue(['gemini']); // Only current provider
      
      const result = await ErrorHandler.handleProviderError(error, 'gemini', 'test');
  ...
  describe('User-Friendly Messages', () => {
    test('provides helpful message for API key errors', () => {
      ...
      const message = ErrorHandler.getUserFriendlyMessage(apiKeyError, 'openai');
    ...
    test('provides helpful message for rate limit errors', () => {
      const rateLimitError = new Error('Rate limit exceeded');
      const message = ErrorHandler.getUserFriendlyMessage(rateLimitError, 'openai');
  ...
  describe('Provider Switching Errors', () => {
    test('handles API key errors during switching', async () => {
      ...
      const result = await ErrorHandler.handleSwitchError(apiKeyError, 'anthropic');
    ...
    test('handles temporary errors during switching', async () => {
      ...
      const result = await ErrorHandler.handleSwitchError(tempError, 'anthropic');
    ...
    test('handles generic errors during switching', async () => {
      ...
      const result = await ErrorHandler.handleSwitchError(genericError, 'anthropic');
