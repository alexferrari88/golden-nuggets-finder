...
describe('Background Script Context Menu', () => {
	...
	beforeEach(() => {
		...
		mockTypeFilterOptions = [
			{ id: 'all', title: '🔍 All Types', types: ['tool', 'media', 'explanation', 'analogy', 'model'] },
		...
		(TypeFilterService.getContextMenuOption as any).mockImplementation((id: string) => 
			mockTypeFilterOptions.find(option => option.id === id)
		);
	...
	describe('Context Menu Creation', () => {
		...
		it('should create basic menu structure without report option by default', async () => {
			...
			const reportCalls = mockContextMenus.create.mock.calls.filter(call =>
				call[0].id === 'report-missed-nugget'
			);
		...
		it('should include report missed nugget option when analysis completed on active tab', async () => {
			...
			mockTabs.query.mockResolvedValue([{ id: tabId, url: 'https://example.com' }]);
