...
describe('Background Script Security - Core Logic Tests', () => {
  ...
  describe('Dynamic Content Script Injection Security', () => {
    ...
    it('should not inject if content script already exists', async () => {
      ...
      expect(mockChrome.tabs.sendMessage).toHaveBeenCalledWith(tabId, { type: "PING" });
    ...
    it('should throw error if injection verification fails', async () => {
      ...
      await expect(injectContentScript(tabId)).rejects.toThrow('Content script failed to inject properly');
    });
    ...
    it('should handle injection API failures', async () => {
      ...
      await expect(injectContentScript(tabId)).rejects.toThrow('Injection failed');
    });
  });
  ...
  describe('Missed Nugget Security Validation', () => {
    ...
    it('should validate tab exists and has ID', () => {
      ...
      // Test undefined tab
      expect(validateMissedNuggetReport(undefined, info, analysisCompletedTabs))
        .toEqual({ valid: false, reason: 'No valid tab' });

      // Test tab without ID
      const tabWithoutId = { url: 'https://example.com' } as chrome.tabs.Tab;
      expect(validateMissedNuggetReport(tabWithoutId, info, analysisCompletedTabs))
        .toEqual({ valid: false, reason: 'No valid tab' });
    });
    ...
    it('should validate analysis completion state', () => {
      ...
      expect(validateMissedNuggetReport(tab, info, emptyTabs))
        .toEqual({ valid: false, reason: 'Analysis not completed' });

      // Test with different tab ID
      const wrongTabTabs = new Set([456]);
      expect(validateMissedNuggetReport(tab, info, wrongTabTabs))
        .toEqual({ valid: false, reason: 'Analysis not completed' });

      // Test undefined tracking set
      expect(validateMissedNuggetReport(tab, info, undefined))
        .toEqual({ valid: false, reason: 'Analysis not completed' });
    });
    ...
    it('should validate selected text length and content', () => {
      ...
      // Test empty text
      expect(validateMissedNuggetReport(tab, { selectionText: '' }, analysisCompletedTabs))
        .toEqual({ valid: false, reason: 'Selected text too short' });

      // Test short text (5 chars or less)
      expect(validateMissedNuggetReport(tab, { selectionText: 'short' }, analysisCompletedTabs))
        .toEqual({ valid: false, reason: 'Selected text too short' });
      ...
      expect(validateMissedNuggetReport(tab, {}, analysisCompletedTabs))
        .toEqual({ valid: false, reason: 'Selected text too short' });

      // Test undefined info
      expect(validateMissedNuggetReport(tab, undefined, analysisCompletedTabs))
        .toEqual({ valid: false, reason: 'Selected text too short' });
    });
    ...
    it('should pass validation with all valid inputs', () => {
      ...
      expect(validateMissedNuggetReport(tab, info, analysisCompletedTabs))
        .toEqual({ valid: true });
    });
    ...
    it('should handle whitespace trimming correctly', () => {
      ...
      const infoWithWhitespace = { selectionText: '   Valid content after trimming   ' };
      expect(validateMissedNuggetReport(tab, infoWithWhitespace, analysisCompletedTabs))
        .toEqual({ valid: true });

      // Text that's too short after trimming
      const infoShortAfterTrim = { selectionText: '   ab   ' };
      expect(validateMissedNuggetReport(tab, infoShortAfterTrim, analysisCompletedTabs))
        .toEqual({ valid: false, reason: 'Selected text too short' });
    });
  });
  ...
  describe('Tab State Management Security', () => {
    ...
    it('should handle concurrent tab operations safely', () => {
      ...
      expect([...analysisCompletedTabs].sort((a, b) => a - b)).toEqual([2, 4, 6, 8, 10]);
    });
  });
  ...
  describe('Input Validation Security', () => {
    ...
    function validateTypeId(typeId: any): boolean {
      const validTypes = ['all', 'tool', 'media', 'explanation', 'analogy', 'model'];
