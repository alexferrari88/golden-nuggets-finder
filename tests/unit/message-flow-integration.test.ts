...
describe('Message Flow Integration Tests', () => {
  ...
  let backgroundMessageHandler: (request: any, sender: any, sendResponse: any) => void;
  let contentMessageHandlers: Map<number, (request: any, sender: any, sendResponse: any) => void>;
  ...
  describe('Background to Content Script Communication', () => {
    it('should handle ANALYZE_CONTENT message flow from context menu to content script', async () => {
      ...
      const response = await mockChrome.tabs.sendMessage(tabId, analysisRequest);
    ...
    it('should handle ENTER_SELECTION_MODE message for content selection workflow', async () => {
      ...
      const contentHandler = vi.fn((request, sender, sendResponse) => {
        ...
        expect(request.typeFilter.selectedTypes).toEqual(['media', 'explanation']);
        sendResponse({ success: true, selectionModeEnabled: true });
      });
      contentMessageHandlers.set(tabId, contentHandler);

      const response = await mockChrome.tabs.sendMessage(tabId, selectionRequest);
    ...
    it('should handle ENTER_MISSING_CONTENT_MODE for missed nugget reporting', async () => {
      ...
      const contentHandler = vi.fn((request, sender, sendResponse) => {
        ...
        expect(request.selectedText).toBe('This is important content that was missed');
        expect(request.url).toBe('https://example.com/article');
        sendResponse({ success: true, missingContentModeEnabled: true });
      });
      contentMessageHandlers.set(tabId, contentHandler);

      const response = await mockChrome.tabs.sendMessage(tabId, missingContentRequest);
    ...
    it('should handle content script injection failures gracefully', async () => {
      ...
      mockChrome.tabs.sendMessage.mockRejectedValueOnce(new Error('No content script available'));

      await expect(mockChrome.tabs.sendMessage(tabId, {
        type: MESSAGE_TYPES.ANALYZE_CONTENT,
        promptId: 'test'
  ...
  describe('Content Script to Background Communication', () => {
    it('should handle analysis progress messages from content script', async () => {
      ...
      expect(backgroundMessageHandler).toHaveBeenCalledWith(progressMessage, sender, sendResponse);
    });
    ...
    it('should handle analysis completion messages with results', async () => {
      ...
      expect(backgroundMessageHandler).toHaveBeenCalledWith(completionMessage, sender, sendResponse);
    });
    ...
    it('should handle analysis error messages with proper error context', async () => {
      ...
      expect(backgroundMessageHandler).toHaveBeenCalledWith(errorMessage, sender, sendResponse);
    });
  });
    ...
    it.skip('should handle complete analysis workflow with progress updates', async () => {
      ...
      const contentHandler = vi.fn((request, sender, sendResponse) => {
        if (request.type === MESSAGE_TYPES.ANALYZE_CONTENT) {
          ...
          const progressSteps = [
            { type: MESSAGE_TYPES.ANALYSIS_CONTENT_EXTRACTED, step: 1, message: 'Content extracted' },
            { type: MESSAGE_TYPES.ANALYSIS_CONTENT_OPTIMIZED, step: 2, message: 'Content optimized' },
            { type: MESSAGE_TYPES.ANALYSIS_API_REQUEST_START, step: 3, message: 'API request started' },
            { type: MESSAGE_TYPES.ANALYSIS_API_RESPONSE_RECEIVED, step: 3, message: 'API response received' },
            { type: MESSAGE_TYPES.ANALYSIS_PROCESSING_RESULTS, step: 4, message: 'Processing results' }
          ];
      ...
      const contentHandler = vi.fn((request, sender, sendResponse) => {
        if (request.type === MESSAGE_TYPES.ANALYZE_CONTENT) {
          ...
          mockChrome.runtime.sendMessage({
            ...
            analysisId,
            context: { step: 'content_processing', details: 'Invalid HTML structure' }
          });
  ...
  describe('Message Validation and Error Handling', () => {
    ...
    it('should validate message types against MESSAGE_TYPES enum', () => {
      ...
      const sendResponse = vi.fn();
      backgroundMessageHandler(invalidMessage, { tab: { id: 1 } }, sendResponse);
