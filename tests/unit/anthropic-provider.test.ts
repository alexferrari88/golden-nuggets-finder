...
describe('LangChainAnthropicProvider', () => {
  ...
  it('should use default model when not specified', () => {
    ...
    const provider = new LangChainAnthropicProvider(configWithoutModel as ProviderConfig);
  ...
  it('should handle API errors gracefully during extraction', async () => {
    ...
    await expect(provider.extractGoldenNuggets('test', 'test'))
      .rejects
      .toThrow('Anthropic API call failed: Anthropic API Error');
  });
  ...
  it('should handle credit balance errors appropriately', async () => {
    ...
    (ChatAnthropic as any).mockImplementationOnce(() => ({
      withStructuredOutput: vi.fn().mockReturnValue({
        invoke: vi.fn().mockRejectedValue(new Error('Your credit balance is too low to access the Anthropic API'))
      })
    }));
    ...
    await expect(provider.extractGoldenNuggets('test', 'test'))
      .rejects
      .toThrow('Anthropic API call failed: Your credit balance is too low to access the Anthropic API');
  });
  ...
  it('should handle rate limit errors appropriately', async () => {
    ...
    await expect(provider.extractGoldenNuggets('test', 'test'))
      .rejects
      .toThrow('Anthropic API call failed: Rate limit exceeded');
  });
