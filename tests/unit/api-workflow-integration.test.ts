...
5. **Mental Models:** Model definition`,
  ...
  describe('Complete Analysis Workflow Integration', () => {
    it.skip('should handle full analysis workflow from request to response', async () => {
      ...
      mockGeminiClient.analyzeContent.mockImplementationOnce(async (content, prompt, options) => {
        // Simulate progress callbacks
        options?.onProgress(MESSAGE_TYPES.ANALYSIS_CONTENT_EXTRACTED, 1, 'Content extracted');
        options?.onProgress(MESSAGE_TYPES.ANALYSIS_CONTENT_OPTIMIZED, 2, 'Content optimized');
        options?.onProgress(MESSAGE_TYPES.ANALYSIS_API_REQUEST_START, 3, 'API request started');
        options?.onProgress(MESSAGE_TYPES.ANALYSIS_API_RESPONSE_RECEIVED, 3, 'API response received');
        options?.onProgress(MESSAGE_TYPES.ANALYSIS_PROCESSING_RESULTS, 4, 'Processing results');
  ...
  describe('Selected Content Analysis Workflow', () => {
    it.skip('should handle selected content analysis with proper message flow', async () => {
      ...
      await messageHandler.handleMessage(selectedContentRequest, sender, sendResponse);

      // Verify analysis was performed
      expect(mockGeminiClient.analyzeContent).toHaveBeenCalledWith(
        'Selected text content for analysis',
        expect.stringContaining('Extract golden nuggets from this Twitter thread'), // Source-specific replacement
        expect.objectContaining({
  ...
  describe('Prompt Management Workflow Integration', () => {
    it.skip('should handle prompt retrieval with optimized prompt integration', async () => {
      ...
      await messageHandler.handleMessage(getPromptsRequest, { tab: { id: 1 } }, sendResponse);
    ...
    it.skip('should handle prompt operations with proper storage integration', async () => {
      ...
      const sendResponse = vi.fn();
      await messageHandler.handleMessage(saveRequest, { tab: { id: 1 } }, sendResponse);
      ...
      await messageHandler.handleMessage(setDefaultRequest, { tab: { id: 1 } }, sendResponse2);
      ...
      await messageHandler.handleMessage(deleteRequest, { tab: { id: 1 } }, sendResponse3);
  ...
  describe('Configuration Management Workflow', () => {
    it.skip('should handle configuration get and save operations', async () => {
      ...
      const sendResponse = vi.fn();
      await messageHandler.handleMessage(getConfigRequest, { tab: { id: 1 } }, sendResponse);
      ...
      const sendResponse2 = vi.fn();
      await messageHandler.handleMessage(saveConfigRequest, { tab: { id: 1 } }, sendResponse2);

      expect(sendResponse2).toHaveBeenCalledWith({ success: true });
    });

    it.skip('should handle options page opening', async () => {
      mockChrome.runtime.openOptionsPage = vi.fn().mockResolvedValueOnce(undefined);
      ...
      const sendResponse = vi.fn();
      await messageHandler.handleMessage(openOptionsRequest, { tab: { id: 1 } }, sendResponse);
  ...
  describe('Error Handling and Validation Integration', () => {
    ...
    it.skip('should handle unknown message types', async () => {
      ...
      const sendResponse = vi.fn();
      await messageHandler.handleMessage(unknownRequest, { tab: { id: 1 } }, sendResponse);
    ...
    it.skip('should handle API client errors during analysis', async () => {
      mockGeminiClient.analyzeContent.mockRejectedValueOnce(new Error('API client initialization failed'));
  ...
  describe('Source Type Detection Integration', () => {
    it.skip('should handle different source types with proper URL detection', async () => {
      ...
      for (const testCase of testCases) {
        ...
        const sendResponse = vi.fn();
        await messageHandler.handleMessage(analysisRequest, { tab: { id: 1 } }, sendResponse);

        // Verify correct source type was used in prompt replacement
        expect(mockGeminiClient.analyzeContent).toHaveBeenCalledWith(
          'Test content',
          expect.stringContaining(`Extract golden nuggets from this ${testCase.expectedSourceType}`),
          expect.objectContaining({
            onProgress: expect.any(Function),
            typeFilter: undefined
          })
