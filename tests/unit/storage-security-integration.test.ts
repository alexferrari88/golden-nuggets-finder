...
describe('Storage-Security Integration Tests', () => {
  ...
  describe('Complete API Key Lifecycle Integration', () => {
    ...
    it.skip('should handle API key updates with proper re-encryption', async () => {
      ...
      const originalStoredData = { ...mockStorageData.get(STORAGE_KEYS.API_KEY) };
      ...
      expect(updatedStoredData.encryptedData).not.toEqual(originalStoredData.encryptedData);
      expect(updatedStoredData.timestamp).toBeGreaterThan(originalStoredData.timestamp);
  ...
  describe('Device Change Recovery Integration', () => {
    it.skip('should handle device change with recovery workflow', async () => {
      ...
      expect(mockChrome.storage.sync.remove).toHaveBeenCalledWith(STORAGE_KEYS.API_KEY);
    });
    ...
    it.skip('should handle recovery failure scenarios gracefully', async () => {
      ...
      mockChrome.storage.sync.remove.mockRejectedValueOnce(new Error('Storage access denied'));
      ...
      expect(securityManager.getAuditLogs().some(log => 
        log.event === 'recovery' && log.success === false
      )).toBe(true);
    });
  });
  ...
  describe('Prompt Storage Integration', () => {
    ...
    it('should handle prompt size limit validation', async () => {
      ...
      await expect(
        storageManager.savePrompts(largePrompts)
      ).rejects.toThrow('Prompt data too large');
  ...
  describe('Configuration Integration', () => {
    ...
    it('should handle partial configuration updates', async () => {
      ...
      await storageManager.saveConfig({
        ...
        userPrompts: [{ id: 'initial', name: 'Initial', prompt: 'Initial content', isDefault: true }]
      ...
      // Update only prompts
      const newPrompts = [
        { id: 'updated', name: 'Updated', prompt: 'Updated content', isDefault: true }
      ];
  ...
  describe('Cache and Performance Integration', () => {
    it.skip('should handle cache consistency across storage operations', async () => {
      ...
      const getCallCountBeforeUpdate = mockChrome.storage.sync.get.mock.calls.length;
      const thirdRetrieval = await storageManager.getApiKey({
        source: 'background',
        action: 'read',
        timestamp: Date.now()
      });
      const getCallCountAfterUpdate = mockChrome.storage.sync.get.mock.calls.length;
    ...
    it('should handle cache expiration properly', async () => {
      const testPrompts = [
        { id: 'cache-test', name: 'Cache Test', prompt: 'Cache test content', isDefault: true }
      ];
  ...
  describe('Error Handling and Recovery Integration', () => {
    it.skip('should handle storage API failures gracefully', async () => {
      ...
      mockChrome.storage.sync.get.mockRejectedValueOnce(new Error('Storage access denied'));
    ...
    it.skip('should handle security manager failures with proper fallback', async () => {
      ...
      vi.spyOn(global.crypto.subtle, 'encrypt').mockRejectedValueOnce(new Error('Encryption failed'));
    ...
    it('should handle complete storage clear operations', async () => {
      ...
      await storageManager.savePrompts([
        { id: 'clear-test', name: 'Clear Test', prompt: 'Clear test content', isDefault: true }
      ]);
