...
import { ResponseNormalizer } from '../../src/background/services/response-normalizer';
import { ProviderId, ProviderConfig } from '../../src/shared/types/providers';
import { z } from 'zod';
...
describe('Multi-Provider Schema Validation Integration Tests', () => {
  ...
  const emptyContent = '';
  const minimalContent = 'This is a test.';
...
Return only the most valuable insights that would be genuinely useful to a software developer.`;
  ...
  beforeAll(() => {
    ...
    if (!hasAnyApiKey) {
      console.warn('⚠️  No API keys found in environment variables. Skipping integration tests.');
      console.warn('   Available keys: GEMINI_API_KEY, OPENAI_API_KEY, ANTHROPIC_API_KEY, OPENROUTER_API_KEY');
    }
  });

  describe.each(['openai', 'anthropic', 'openrouter'] as ProviderId[])('%s provider', (providerId) => {
    const hasApiKey = !!apiKeys[providerId];

    it.skipIf(!hasApiKey)('should return valid JSON response matching golden nuggets schema', async () => {
      ...
      try {
        ...
        const rawResponse = await provider.extractGoldenNuggets(testContent, testPrompt);
        console.log(`API call successful for ${providerId}`);
        
        // Normalize response to handle provider-specific quirks
        response = ResponseNormalizer.normalize(rawResponse, providerId);
        ...
        const validationResult = GoldenNuggetsResponseSchema.safeParse(response);
        if (!validationResult.success) {
          console.error(`Schema validation failed for ${providerId}:`, validationResult.error);
        ...
        if (response.golden_nuggets.length > 0) {
          response.golden_nuggets.forEach((nugget, index) => {
        ...
        console.log(`✅ ${providerId}: Successfully extracted ${response.golden_nuggets.length} golden nuggets`);
      } catch (error) {
  ...
  describe('Response validation utilities', () => {
    it('should validate correct response format', () => {
      ...
      const isValid = ResponseNormalizer.validate(validResponse);
    ...
    it('should reject invalid response format', () => {
      ...
      const isValid = ResponseNormalizer.validate(invalidResponse);
    ...
    it('should handle malformed response gracefully', () => {
      ...
      const isValid = ResponseNormalizer.validate(malformedResponse);
      expect(isValid).toBe(false);
    });
  });
