...
test.describe('Feedback Reset Flow', () => {
  ...
  test.skip('should allow user to reset feedback on golden nuggets', async ({ page }) => {
  ...
  test.skip('should handle reset confirmation modal', async ({ context, extensionId }) => {
    ...
    await page.evaluate(() => {
      ...
      resetBtn.style.cssText = 'padding: 8px 16px; border: 1px solid #007cba; border-radius: 4px; background: #007cba; color: white; cursor: pointer;';
    ...
    await expect(page.locator('#reset-confirmation-overlay')).not.toBeAttached();
    ...
    await page.evaluate(() => {
      ...
      const description = document.createElement('p');
      description.textContent = 'Are you sure you want to reset your feedback for this nugget?';
  ...
  // Note: This test is skipped due to limitations with testing content script UI in Playwright  
  // UI behavior testing is better handled in unit tests for content script components
  test.skip('should handle reset button visibility', async ({ context, extensionId }) => {
    const page = await context.newPage();
    ...
    await expect(resetBtn).toHaveAttribute('title', 'Reset feedback - removes rating and type correction');
