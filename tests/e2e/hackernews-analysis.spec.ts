...
test.describe('HackerNews Analysis', () => {
  test('can load and analyze HackerNews discussion page', async ({ context, extensionId }) => {
    ...
    try {
      ...
      await page.screenshot({ path: 'tests/debug-hackernews-page.png', fullPage: true });
  ...
  test('can simulate extension analysis on HackerNews page', async ({ context, extensionId }) => {
    const page = await context.newPage();
    ...
    try {
      ...
      const result = await page.evaluate(() => {
        ...
        const commentElements = document.querySelectorAll('.comment-tree .comment, .athing.comtr .commtext');
        
        const storyText = storyElement?.textContent || '';
        const commentTexts = Array.from(commentElements).map(el => el.textContent || '').slice(0, 5); // First 5 comments
      ...
      console.log('HackerNews content analysis:', JSON.stringify(result, null, 2));
      ...
      await page.screenshot({ path: 'tests/debug-hackernews-analysis.png', fullPage: true });
      throw error;
    } finally {
      await page.close();
    }
  ...
  test('can test HackerNews content structure for highlighting', async ({ context, extensionId }) => {
    const page = await context.newPage();
    ...
    try {
      ...
      const result = await page.evaluate(() => {
        ...
        const storyTitle = document.querySelector('.titleline a')?.textContent || '';
        const storyMetadata = document.querySelector('.subtext')?.textContent || '';
        const comments = document.querySelectorAll('.commtext');
        
        // Check if we can find highlightable content
        const commentTexts = Array.from(comments).map(comment => {
        ...
        return {
          ...
          substantialComments: commentTexts.filter(c => c.hasSubstantialText).length,
          commentSamples: commentTexts
        };
      });
      
      console.log('HackerNews structure analysis:', JSON.stringify(result, null, 2));
