...
test.describe('Extension Basics', () => {
  test('extension loads and gets service worker', async ({ context, extensionId }) => {
  ...
  test('extension pages are accessible', async ({ context, extensionId }) => {
    ...
    await expect(popupPage).toHaveURL(new RegExp(`chrome-extension://${extensionId}/popup.html`));
