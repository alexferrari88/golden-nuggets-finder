...
test.describe("Options Page", () => {
	...
	test("handles model selection workflow", async ({
		context,
		extensionId,
	}) => {
		const page = await context.newPage();
		...
		const stateIndicators = page.locator('.loading, [aria-busy="true"], .spinner');
	...
	test("preserves existing functionality while adding model selection", async ({
		...
		const promptSections = page.locator('[data-testid*="prompt"], [data-testid*="custom"]');
