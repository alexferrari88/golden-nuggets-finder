...
export class MessageHandler {
		...
		try {
			while (attempts < maxAttempts) {
				...
				try {
					...
					const provider = await providerService.createProvider(providerConfig.providerId, providerConfig.modelName);
	...
	private async handleMultiProviderEnsemble(
		...
		const validation = providerService.validateProviderConfigurations(
			ensembleOptions.providerConfigurations,
		);
		if (!validation.valid) {
			throw new Error(
		...
		const providerInstances = await providerService.createMultipleProviders(
			ensembleOptions.providerConfigurations,
		);
	...
	private async handleSingleModelEnsemble(
		...
		// Create provider instance
		const providerConfig = await MessageHandler.getSelectedProvider();
		const providerInstance = await providerService.createProvider(providerConfig.providerId, providerConfig.modelName);
	...
	private async handleValidateProvider(
		...
		try {
			...
			const provider = await providerService.createProvider(config.providerId, config.modelName);
