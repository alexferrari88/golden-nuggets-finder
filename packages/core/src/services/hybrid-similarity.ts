...
// Default configuration constants (copied from original shared/constants)
...
export class HybridSimilarityMatcher {
	...
	constructor(
		private options: Partial<SimilarityOptions> = {},
		private embeddingService?: EmbeddingServiceInterface,
		private logger?: LoggerInterface,
	) {
		// Disable embeddings if service not available
		if (this.options.useEmbeddings && !this.embeddingService) {
			this.options.useEmbeddings = false;
			this.logger?.warn(
				"[HybridSimilarityMatcher] Embedding service not available, disabling embeddings",
			);
		}
	}
	...
	async calculateSimilarity(
		...
		if (config.useEmbeddings && this.embeddingService) {
			try {
				...
				return {
					similarity: score,
					method: "embedding",
					isSimilar: score >= (config.embeddingThreshold ?? 0.8),
					metadata: { embeddingScore: score },
				};
			} catch (error) {
				const errorMessage =
				...
				return {
					similarity: score,
					method: "fallback",
					isSimilar: score >= (config.wordOverlapThreshold ?? 0.8),
					metadata: {
						embeddingError: errorMessage,
						wordOverlapScore: score,
					},
				};
			}
		}
		...
		return {
			...
			method: "word_overlap",
			isSimilar: score >= (config.wordOverlapThreshold ?? 0.8),
			metadata: { wordOverlapScore: score },
		};
	}
			...
			for (let j = i + 1; j < enrichedNuggets.length; j++) {
				...
				if (
					...
					try {
						...
						isSimilar = embeddingScore >= (config.embeddingThreshold ?? 0.8);
					} catch (error) {
						this.logger?.warn(
						...
						isSimilar =
							wordOverlapScore >= (config.wordOverlapThreshold ?? 0.8);
					}
				} else {
					...
					isSimilar = wordOverlapScore >= (config.wordOverlapThreshold ?? 0.8);
				}
